...
export default function Home() {
  ...
  const [goudiContributionAmount, setGoudiContributionAmount] = useState("");
  ...
  const confirmGoudi = () => {
    ...
    const contributionExpected = Number(goudiContributionAmount);
    if (!hasMemberSession || !Number.isInteger(accountId)) return;
    if (!Number.isInteger(contributionExpected) || contributionExpected <= 0) {
      setAccountNotice("Renseignez un montant de contribution valide avant de confirmer le Goudi Adjouma.");
      return;
    }
    ...
    <div className="dahira-shell min-h-screen pb-24 lg:pb-0">
      <div className="mx-auto flex min-h-screen max-w-[1540px]">
        ...
        <main className="min-w-0 flex-1 px-4 py-4 sm:px-6 sm:py-6 lg:px-9 lg:py-8">
          ...
          {view === "goudi" && <GoudiPage role={role} members={members} history={persistedGoudiQuery.data?.events ?? []} organizer={organizer} suggestedOrganizer={members.find(member => member.id === suggestedOrganizer?.id) ?? organizer} organizerId={organizerId} setOrganizerId={setOrganizerId} contributionAmount={goudiContributionAmount} setContributionAmount={setGoudiContributionAmount} suggestionConfirmed={suggestionConfirmed} setSuggestionConfirmed={setSuggestionConfirmed} notificationsEnabled={notificationsEnabled} onEnableNotifications={() => void requestNotifications()} onConfirm={confirmGoudi} />}
...
function Dashboard({ role, hasMemberSession, isMemberPreview, organizer, metrics, checkInOpen, notificationsEnabled, notificationFeedback, onNavigate, onEnableNotifications }: { role: AppRole; hasMemberSession: boolean; isMemberPreview: boolean; organizer: Member; metrics: { memberCount: number; received: number; expected: number; treasury: number; attendanceCount: number; paidCount: number; pendingCount: number; lateCount: number }; checkInOpen: boolean; notificationsEnabled: boolean; notificationFeedback: string; onNavigate: (view: View) => void; onEnableNotifications: () => void }) {
  return <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
    ...
    {isMemberPreview && <div className="mt-4 rounded-2xl border border-[#cfe4d3] bg-[#eef7ef] p-4 text-sm leading-6 text-[#35634a]"><b>Aperçu espace membre.</b> Le membre accède à son pointage, ses cotisations, le prochain Goudi Adjouma et ses notifications ; la caisse et les données des autres membres restent privées.</div>}
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
      ...
      <section className="surface-card p-5 sm:p-6"><SectionHeading eyebrow="Communication" title="Rester informé" /><div className="flex items-center justify-between gap-4 rounded-3xl bg-[#f0f7f1] p-4"><div className="flex items-start gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-white text-[#0c5a48] shadow-sm"><Bell className="h-5 w-5" /></span><div><p className="font-bold text-[#1c4c3e]">Notifications web</p><p className="mt-1 text-sm leading-5 text-[#698075]">Rappels de Goudi Adjouma et informations utiles, sans SMS.</p></div></div><button onClick={onEnableNotifications} className={`shrink-0 rounded-xl px-3 py-2 text-xs font-bold ${notificationsEnabled ? "bg-[#0c5a48] text-white" : "bg-white text-[#0c5a48] shadow-sm"}`}>{notificationsEnabled ? "Activées" : "Activer"}</button></div>{notificationFeedback && <p className="mt-3 rounded-2xl bg-[#f8f4e8] px-3 py-2 text-xs leading-5 text-[#6e755f]">{notificationFeedback}</p>}<div className="mt-4 flex items-center gap-3 text-xs text-[#75877e]"><ShieldCheck className="h-4 w-4 text-[#0c5a48]" />Les membres décident eux-mêmes d’accepter les alertes.</div></section>
    </div>
  </div>;
}
...
function GoudiPage({ role, members, history, organizer, suggestedOrganizer, organizerId, setOrganizerId, contributionAmount, setContributionAmount, suggestionConfirmed, setSuggestionConfirmed, notificationsEnabled, onEnableNotifications, onConfirm }: { role: AppRole; members: Member[]; history: { id: number; organizerAccountId: number | null; scheduledFor: Date; status: "proposed" | "confirmed" | "completed" }[]; organizer: Member; suggestedOrganizer: Member; organizerId: string; setOrganizerId: (id: string) => void; contributionAmount: string; setContributionAmount: (value: string) => void; suggestionConfirmed: boolean; setSuggestionConfirmed: (value: boolean) => void; notificationsEnabled: boolean; onEnableNotifications: () => void; onConfirm: () => void }) {
  const canManageGoudi = role === "admin" || role === "treasurer";
  const isAutomaticSuggestion = organizer.id === suggestedOrganizer.id;
  const enteredContribution = Number(contributionAmount);
  const contributionLabel = Number.isInteger(enteredContribution) && enteredContribution > 0 ? money(enteredContribution) : "À renseigner";
  return <div className="animate-in fade-in slide-in-from-bottom-2 duration-300"><PageIntro eyebrow="Goudi Adjouma" title="Préparer jeudi prochain avec sérénité." description="L’application propose une rotation juste ; l’administrateur ou le trésorier garde toujours le dernier mot." /><div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]"><section className="surface-card overflow-hidden"><div className="bg-[#073d32] p-5 text-white sm:p-6"><div className="flex items-start justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[#efcf7d]">Proposition pour jeudi 3 septembre</p><h2 className="display mt-2 text-3xl">{organizer.name}</h2><p className="mt-2 text-sm text-white/66">{isAutomaticSuggestion ? `Suggestion automatique · rotation #${organizer.rotation}` : `Choix manuel · suggestion initiale : ${suggestedOrganizer.name}`}</p></div><span className="grid h-11 w-11 place-items-center rounded-2xl bg-white/10"><Sparkles className="h-5 w-5 text-[#f4cd79]" /></span></div></div><div className="p-5 sm:p-6"><div className="mb-5 flex items-center gap-3"><Avatar member={organizer} size="lg" /><div><p className="font-bold text-[#1b4a3c]">{organizer.phone}</p><p className="mt-1 text-sm text-[#75877e]">Contribution prévue : <b className="text-[#1b4a3c]">{contributionLabel}</b></p></div></div>{canManageGoudi ? <><label className="block text-sm font-bold text-[#264d41]">Modifier l’organisateur<select value={organizerId || suggestedOrganizer.id} onChange={event => { setOrganizerId(event.target.value); setSuggestionConfirmed(false); }} className="mt-2 h-12 w-full rounded-xl border border-[#e5ddcd] bg-white px-3 text-sm font-semibold text-[#264d41] outline-none focus:border-[#0c5a48] focus:ring-4 focus:ring-[#dceee1]">{members.map(member => <option value={member.id} key={member.id}>{member.name} — rotation #{member.rotation}</option>)}</select></label><label className="mt-4 block text-sm font-bold text-[#264d41]">Montant de contribution (F CFA)<input required min="1" value={contributionAmount} onChange={event => { setContributionAmount(event.target.value); setSuggestionConfirmed(false); }} placeholder="Ex. 5 000" inputMode="numeric" className="mt-2 h-12 w-full rounded-xl border border-[#e5ddcd] bg-white px-3 text-sm font-semibold text-[#264d41] outline-none focus:border-[#0c5a48] focus:ring-4 focus:ring-[#dceee1]" /></label></> : <div className="rounded-2xl bg-[#f4f5ef] p-4 text-sm leading-6 text-[#688077]">Seul l’administrateur ou le trésorier peut confirmer l’organisateur et renseigner le montant.</div>} {canManageGoudi && <div className="mt-5 grid gap-3 sm:grid-cols-2"><button onClick={onConfirm} className={`soft-button ${suggestionConfirmed ? "bg-[#e2f0e5] text-[#247044]" : "bg-[#073d32] text-white"}`}>{suggestionConfirmed ? <><Check className="h-4 w-4" />Organisateur confirmé</> : "Confirmer le choix"}</button><button onClick={onEnableNotifications} className="soft-button border border-[#dfd9cc] bg-white text-[#0c5a48]"><Send className="h-4 w-4" />{notificationsEnabled ? "Rappel prêt" : "Préparer le rappel"}</button></div>}</div></section><section className="surface-card p-5 sm:p-6"><SectionHeading eyebrow="Historique" title="Derniers organisateurs" /><div className="space-y-4">{history.length === 0 ? <p className="rounded-2xl bg-[#fbfaf5] p-4 text-sm text-[#7b8c82]">Aucun Goudi Adjouma enregistré pour le moment.</p> : history.slice().sort((a, b) => +new Date(b.scheduledFor) - +new Date(a.scheduledFor)).map(entry => { const member = members.find(item => item.id === `member-${entry.organizerAccountId}`); if (!member) return null; return <div key={entry.id} className="flex items-center gap-3"><Avatar member={member} size="sm" /><div className="min-w-0 flex-1"><p className="text-sm font-bold text-[#214b3d]">{member.name}</p><p className="mt-1 text-xs text-[#7b8c82]">{new Date(entry.scheduledFor).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</p></div><span className="rounded-full bg-[#edf4ed] px-2.5 py-1 text-[10px] font-bold text-[#397650]">{entry.status === "confirmed" ? "Confirmé" : entry.status === "completed" ? "Terminé" : "Proposé"}</span></div>; })}</div><div className="mt-6 rounded-2xl bg-[#fbf4e4] p-4"><p className="text-sm font-bold text-[#76591f]">Une rotation équitable</p><p className="mt-1 text-xs leading-5 text-[#8b7445]">La suggestion suit les membres actifs et le dernier organisateur confirmé. À la fin de la liste, elle reprend au premier membre actif.</p></div></section></div></div>;
}
